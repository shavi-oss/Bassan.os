...
describe('🔓 Security Penetration Tests', () => {
  ...
  describe('🎯 IDOR Attacks (Insecure Direct Object Reference)', () => {
    ...
    it('ATTACK: Update victim lead by directly using their ID', async () => {
      ...
      const lead = await prisma.lead.findUnique({ where: { id: victimLeadId } });
  ...
  describe('💉 organizationId Injection Attacks', () => {
    ...
    it('ATTACK: Inject victim orgId in query string', async () => {
      ...
      const victimLeaks = leads.filter((l: any) => l.organizationId === victimOrgId);
  ...
  describe('🔐 Authentication Bypass Attacks', () => {
    ...
    it('ATTACK: Use invalid/expired token', async () => {
      ...
      const fakeToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwib3JnYW5pemF0aW9uSWQiOiJ2aWN0aW0tb3JnLWlkIiwiaWF0IjoxNTE2MjM5MDIyfQ.4S5J9';
    ...
    it('ATTACK: Modify token payload to access victim org', async () => {
      ...
      const tamperedToken = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJhdHRhY2tlci1pZCIsIm9yZ2FuaXphdGlvbklkIjoidmljdGltLW9yZy1pZCIsImVtYWlsIjoiYXR0YWNrZXJAZXZpbC5jb20ifQ.invalid';
