...
export class PrismaService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(private readonly tenantExtension: PrismaTenantExtension) {
    ...
    this._prisma.$on(
      ...
      (event: { query: string; params: string; duration: number }) => {
        ...
        const securityStatus = hasTenantFilter
          ? "✅ FILTERED"
          : "⚠️ UNFILTERED";
  ...
  async onModuleInit() {
    ...
    this.logger.log(
      "🔒 PrismaService using COMPOSITION pattern - base client not exposed",
    );
  }
  ...
  get client() {
    if (!this._secureClient) {
      throw new Error(
        "PrismaService not initialized - call onModuleInit first",
      );
    }
  ...
  $queryRaw(): never {
    ...
    throw new Error(
      "SECURITY_VIOLATION: Raw SQL queries are forbidden. Use prisma.client methods.",
    );
  }
  ...
  get _unsafeClient(): PrismaClient {
    this.logger.warn(
      "⚠️ [SECURITY] Accessing _unsafeClient - ensure this is justified!",
    );
    return this._prisma;
  }
}
