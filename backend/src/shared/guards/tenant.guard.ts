...
export class TenantGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!user) {
      this.logger.error(
        "🚨 SECURITY: No user in request - JwtAuthGuard did not run",
      );
    ...
    if (!user.organizationId) {
      this.logger.error(
        `🚨 SECURITY: No organizationId in JWT for user ${user.id}`,
      );
      throw new UnauthorizedException(
        "Invalid token - no organization context",
      );
    }
  ...
  private sanitizeRequest(req: Request): void {
    ...
    if (req.query) {
      if (req.query.organizationId) {
        this.logger.warn(
          `⚠️ BLOCKED: Manual organizationId in query: ${req.query.organizationId}`,
        );
        delete req.query.organizationId;
      }
      if (req.query.orgId) {
        this.logger.warn(
          `⚠️ BLOCKED: Manual orgId in query: ${req.query.orgId}`,
        );
        delete req.query.orgId;
      }
      if (req.query.tenantId) {
        this.logger.warn(
          `⚠️ BLOCKED: Manual tenantId in query: ${req.query.tenantId}`,
        );
    ...
    if (req.body && typeof req.body === "object") {
      if (req.body.organizationId) {
        this.logger.warn(
          `⚠️ BLOCKED: Manual organizationId in body: ${req.body.organizationId}`,
        );
      ...
      if (req.body.tenantId) {
        this.logger.warn(
          `⚠️ BLOCKED: Manual tenantId in body: ${req.body.tenantId}`,
        );
    ...
    if (req.params) {
      if (req.params.organizationId) {
        this.logger.warn(
          `⚠️ BLOCKED: Manual organizationId in params: ${req.params.organizationId}`,
        );
